...
class ListDumps : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        with(rootElement) {
            ...
            vehicleRecycler.layoutManager = LinearLayoutManager(this@ListDumps)
        }

        setupAuthGuard {
            mViewModel = initViewModel()
            mViewModel.initDumps()
    ...
    private fun dataItemClick(dataItem: DataVehicles) {
        ...
        intent.putExtra("volumeDump", dataItem.volumeDump)
        intent.putExtra("massDump", dataItem.massDump)
