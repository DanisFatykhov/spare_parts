...
class ListAutocranes : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        with(rootElement) {
            ...
            vehicleRecycler.layoutManager = LinearLayoutManager(this@ListAutocranes)
        }


        setupAuthGuard {
            mViewModel = initViewModel()
            mViewModel.initAutocranes()
    ...
    private fun dataItemClick(dataItem: DataVehicles) {
        ...
        intent.putExtra("heightArrow", dataItem.heightArrow)
        intent.putExtra("massArrow", dataItem.massArrow)
